    ...
    return (
            <section id="projects" className="max-w-5xl mx-auto px-10 py-16">
                <div className="grid grid-cols-2 gap-3">
                    {projects.map((p)=>(
                        <div key={p.title} className="bg-white border border-purple-100 rounded-2xl p-6
                         hover:-translate-y-1 transition-transform">
                        <div className="text-4xl mb-3">{p.emoji}</div>
                        <h3 className="font-serif text-1g font-bold mb-2">{p.title}</h3>
                        <p className="text-sm text-purple-400 mb-4">{p.desc}</p>
                        <div className="flex flex-wrap gap-2"> {p.tags.map(tag=>(
                                       <span key={tag} className="text-xs font-semibold px-2 py-1 rounded-full bg-berry text-apricot mb-4">{tag}</span>
                        ))}
                        </div>
                        </div>
                    ))}
